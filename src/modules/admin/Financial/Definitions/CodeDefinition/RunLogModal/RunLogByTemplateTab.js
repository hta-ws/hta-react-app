...
const RunLogByTemplateTab = ({ templateId }) => {
  const runAction = 'run-fs-code-definition-job-by-template';
  const [apiStates, apiActions] = useGetDataApi({
    controller: 'definition',
    action: 'get-fs-code-definition-run-log-by-template-list',
    method: 'POST',
    initialData: [],
  });
  ...
  const params = { fs_template_id: templateId };

  useEffect(() => {
    setQueryParams(params);
  }, [templateId, setQueryParams]);
...
RunLogByTemplateTab.propTypes = {
  templateId: PropTypes.number.isRequired,
};

export default RunLogByTemplateTab;
